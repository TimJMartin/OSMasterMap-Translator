...
DELETE FROM ${update_schema~}.cartographicsymbol WHERE toid IN (SELECT toid FROM ${schema~}.cartographicsymbol);
COMMIT;

DELETE FROM ${update_schema~}.cartographicsymbol WHERE toid IN (SELECT toid FROM ${schema~}.departedfeature);
COMMIT;

INSERT INTO ${update_schema~}.cartographicsymbol SELECT * FROM ${schema~}.cartographicsymbol;
COMMIT;

COMMENT ON TABLE ${update_schema~}.cartographicsymbol IS ${release};
COMMIT;