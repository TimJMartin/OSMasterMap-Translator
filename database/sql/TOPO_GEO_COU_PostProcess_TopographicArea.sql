...
ALTER TABLE ${schema~}.topographicarea RENAME COLUMN fid TO toid;
COMMIT;

ALTER TABLE ${schema~}.topographicarea ADD PRIMARY KEY (toid);
COMMIT;

DELETE FROM ${update_schema~}.topographicarea WHERE toid IN (SELECT toid FROM ${schema~}.topographicarea);
COMMIT;

DELETE FROM ${update_schema~}.topographicarea WHERE toid IN (SELECT toid FROM ${schema~}.departedfeature);
COMMIT;

INSERT INTO ${update_schema~}.topographicarea SELECT * FROM ${schema~}.topographicarea;
COMMIT;

COMMENT ON TABLE ${update_schema~}.topographicarea IS ${release};
COMMIT;