...
CREATE INDEX fid_${schema^}_topographicline_idx ON ${schema~}.topographicline(fid);
COMMIT;

DELETE FROM ${schema~}.topographicline a WHERE a.ctid <> (SELECT min(b.ctid) FROM ${schema~}.topographicline b WHERE  a.fid = b.fid);
COMMIT;
...
ALTER TABLE ${schema~}.topographicline RENAME COLUMN fid TO toid;
COMMIT;

ALTER TABLE ${schema~}.topographicline ADD PRIMARY KEY (toid);
COMMIT;

DELETE FROM ${update_schema~}.topographicline WHERE toid IN (SELECT toid FROM ${schema~}.topographicline);
COMMIT;

DELETE FROM ${update_schema~}.topographicline WHERE toid IN (SELECT toid FROM ${schema~}.departedfeature);
COMMIT;

INSERT INTO ${update_schema~}.topographicline SELECT * FROM ${schema~}.topographicline;
COMMIT;

COMMENT ON TABLE ${update_schema~}.topographicline IS ${release};
COMMIT;