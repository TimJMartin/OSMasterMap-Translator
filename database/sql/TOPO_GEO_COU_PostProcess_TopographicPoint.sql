...
CREATE INDEX fid_${schema^}_topographicpoint_idx ON ${schema~}.topographicpoint(fid);
COMMIT;

DELETE FROM ${schema~}.topographicpoint a WHERE a.ctid <> (SELECT min(b.ctid) FROM ${schema~}.topographicpoint b WHERE  a.fid = b.fid);
COMMIT;
...
ALTER TABLE ${schema~}.topographicpoint RENAME COLUMN fid TO toid;
COMMIT;

ALTER TABLE ${schema~}.topographicpoint ADD PRIMARY KEY (toid);
COMMIT;

DELETE FROM ${update_schema~}.topographicpoint WHERE toid IN (SELECT toid FROM ${schema~}.topographicpoint);
COMMIT;

DELETE FROM ${update_schema~}.topographicpoint WHERE toid IN (SELECT toid FROM ${schema~}.departedfeature);
COMMIT;

INSERT INTO ${update_schema~}.topographicpoint SELECT * FROM ${schema~}.topographicpoint;
COMMIT;

COMMENT ON TABLE ${update_schema~}.topographicpoint IS ${release};
COMMIT;
