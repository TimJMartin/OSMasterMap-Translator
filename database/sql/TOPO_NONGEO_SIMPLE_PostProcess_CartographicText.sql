...
ALTER TABLE ${schema~}.cartographictext ADD COLUMN id SERIAL PRIMARY KEY;
COMMIT;

CREATE INDEX geom_${schema^}_cartographictext_idx ON ${schema~}.cartographictext USING GIST (geom);
COMMIT;

CREATE INDEX stylecode_${schema^}_cartographictext_idx ON ${schema~}.cartographictext(style_code);
COMMIT;

COMMENT ON TABLE ${schema~}.cartographictext IS ${release};
COMMIT;